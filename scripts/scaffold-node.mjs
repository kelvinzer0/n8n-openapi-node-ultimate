...
const DEPRECATED_PREFIXES = [
        { prefix: 'amazonaws', platform: 'Amazon Web Services (AWS)', officialNode: 'https://docs.n8n.io/integrations/builtin/app-nodes/n8n-nodes-base.aws/' },
...
if (deprecatedInfo) {
        console.log(`  ⚠️  DEPRECATED: ${deprecatedInfo.platform} — use official n8n node instead`);
        console.log(`  📎 Official: ${deprecatedInfo.officialNode}`);
}
...
// Build deprecation fields for deprecated platforms (Amazon, Google, Microsoft)
const deprecatedFields = deprecatedInfo
        ? `,
                deprecated: true,
                deprecationNotice: 'This community node is deprecated. Use the official ${deprecatedInfo.platform} node built into n8n instead. See: ${deprecatedInfo.officialNode}',`
        : '';

// Build deprecated description prefix
const deprecatedDescPrefix = deprecatedInfo
        ? `[DEPRECATED] Use the official ${deprecatedInfo.platform} node built into n8n. `
        : '';
...
export class ${nodeClassName} implements INodeType {
        description: INodeTypeDescription = {
                displayName: '${escapeTS(toDisplayName(CUSTOM_NAME))}${deprecatedInfo ? ' (Deprecated)' : ''}',
                name: '${nodeInternalName}',
                icon: { light: 'file:./${actualIconLight}', dark: 'file:./${actualIconDark}' },
                group: ['input'],
                version: 1,
                subtitle: '={{\\$parameter["operation"] + ": " + \\$parameter["resource"]}}',
                description: '${escapeTS(deprecatedDescPrefix + defaultDesc)}',${deprecatedFields}
                defaults: { name: '${escapeTS(toDisplayName(CUSTOM_NAME))}' },
                usableAsTool: ${deprecatedInfo ? 'false' : 'true'},
...
// Build deprecation banner for README
const deprecatedReadmeBanner = deprecatedInfo ? `
> **⚠️ DEPRECATED — This node is no longer maintained.**
>
> n8n has **built-in ${deprecatedInfo.platform} nodes** that are officially supported, regularly updated, and offer better UX.
>
> **Use the official ${deprecatedInfo.platform} node instead:**
> ${deprecatedInfo.officialNode}
>
...
writeFileSync(
        ...
        `# ${packageName}${deprecatedInfo ? ' *(Deprecated)*' : ''}

![${CUSTOM_NAME} Banner](banner.svg)

[![npm version](https://img.shields.io/npm/v/${packageName}.svg)](https://www.npmjs.com/package/${packageName})${deprecatedInfo ? ' [![Deprecated](https://img.shields.io/badge/Status-Deprecated-red.svg)](' + deprecatedInfo.officialNode + ')' : ''}
[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](https://opensource.org/licenses/MIT)

---
${deprecatedReadmeBanner}
